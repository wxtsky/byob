import { GoBackInput } from '@byob/shared';
...
export async function handleGoBack(
  ...
  const params = GoBackInput.parse(rawParams);
  throwIfAborted(signal);

  // chrome.tabs.goBack rejects if there's nothing to go back to ("Cannot find
  // a next page in history."). Convert that into our no_history envelope.
  try {
    await chrome.tabs.goBack(params.tabId);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (/no.*history|cannot find/i.test(msg)) {
      return { error: 'no_history', message: 'No history to go back to in this tab.' };
    }
